...
\begin{research}
  ...
  \description{
    - We formulated the distributed and cooperative job dispatching problem with outdated nd partial information as a POMDP problem. \\
...
\begin{section}{Projects}      
    \begin{project}
        \title{Manipulation of Networking Stack in Linux Kernel} %Netfilter + Netlink + mac80211
        \duration{Oct '16 - Oct '18}
        \description{
          - \cusemph{MAC Layer (L2)}: (\href{http://github.com/iamhyc/wlsops-hack}{Github Link})
          \begin{itemize}
            \item[--] This project aims at providing the fastest access to the wireless NIC driver from userspace.
            ...
            We design one kernel module to hijack the function entry of the driver, and use ``mmap'' to establish a shared-memory communication between kernel and userspace.
            \item[--] The experiment results show that we could alter a set of parameters (related to channel access priority) within 10 millisecond for 1000 accesses. 
          \end{itemize}
          ...
          \begin{itemize}
            ...
            It uses \cusemph{asynchronous encryption method AES-128} provided by kernel function, to encrypt the payload of L3 (i.e., content of an IP packet) and decrypt it correspondingly at the receiver’s side.
    ...
    \begin{project}
      ...
      \description{
        ...
        - We implemented the system on NI instruments with out-of-shelf Wi-Fi NIC, and the retransmission mechanism is implemented on IP layer. The source code of the system implementation is provided on \href{https://github.com/iamhyc/CSDS-VLC-TCP}{Github}. We also have one granted patent (CN110429979B) over this platform.
      }
    \end{project}
    \\
    \begin{project}
      ...
      \description{
        - This IPC framework is writen in memory-safe Rust language. \cusemph{SerDe} stands for serializing-and-deserializing, which is also the name of a crate developed in Rust. \\
        - This IPC framework borrows the paradigms from \emph{Android Binder} and \emph{gRPC}, which aims at providing a unified FFI access for \cusemph{any typed-function signatures in any languages}. It currently supports Rust/CPP/Python services. \\
        ...
        - This IPC framework currently serves as a component of \cusemph{VDM Capability Library} which aims at gathering all best system-related practices. The source code is available on \href{https://github.com/VDM-Maintainer-Group/vdm-capability-library}{Github}.
      }
  \end{project}
